...
def extract_name_from_resume(text):
    ...
    # This pattern assumes names with a capital letter followed by lowercase letters, optionally including middle names or initials
    pattern = r"(?<!\S)([A-Z][a-z]*\s[A-Z][a-z]*(\s[A-Z][a-z]*)?)(?!\S)"
    
    matches = re.findall(pattern, text)
    if matches:
        # Select the first matched name (which is often a safe assumption in resumes)
        name = matches[0][0]
